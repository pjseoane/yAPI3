...
def auto_report(
    ...
    if include_factors:
        ...
        r.add_text(
            ...
            "* p&lt;0.1  ** p&lt;0.05  *** p&lt;0.01. "
            "Rolling betas (126-day window) show how factor exposures shift across regimes."
        )

        # per-symbol: beta loadings + summary
        factor_results = []
        for sym in symbols:
            try:
        ...
        if len(factor_results) > 1:
            try:
                r.add_plot(plots.factor_comparison(factor_results))
            except Exception as e:
                r.add_text(f"<i>Factor comparison unavailable: {e}</i>")

        # rolling betas — how exposures shift over time
        for sym in symbols:
            try:
                r.add_plot(plots.rolling_factor_betas(
                    quant, sym,
                    model=factor_model,
                    period=period,
                    window=126,
